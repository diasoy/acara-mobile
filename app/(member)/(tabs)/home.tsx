...
const quickActions = [
  {
    ...
    href: "/(member)/ticket" as const,
  },
  {
    id: "history",
    title: "Riwayat",
    icon: "time-outline" as const,
    href: "/(member)/history" as const,
  },
];
  ...
  return (
    ...
    >
      <ScrollView className="flex-1" contentContainerClassName="px-5 pb-8">
        <View className="pt-2">
          <Text
            className={`font-manrope text-sm ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}
          >
            Selamat datang kembali,
          </Text>
          <Text
            className={`font-manrope-bold text-2xl ${isDarkMode ? "text-white" : "text-slate-900"}`}
          >
        ...
        <View className="mt-5 rounded-2xl bg-emerald-500 p-5">
          <Text className="font-manrope-semibold text-slate-900">
            Member Benefit
          </Text>
        ...
        <Text
          className={`mt-6 font-manrope-bold text-xl ${isDarkMode ? "text-white" : "text-slate-900"}`}
        >
          Menu Cepat
        </Text>
        <View className="mt-3 flex-row flex-wrap justify-between">
          {quickActions.map((action) => (
            <Link key={action.id} href={action.href as any} asChild>
              <Pressable
                className={`mb-3 w-[48.5%] rounded-2xl border p-4 active:opacity-85 ${isDarkMode ? "border-slate-800 bg-slate-900" : "border-slate-200 bg-white"}`}
              >
                <View
        ...
        <Text
          className={`mt-3 font-manrope-bold text-xl ${isDarkMode ? "text-white" : "text-slate-900"}`}
        >
          Rekomendasi Event
        </Text>
        <View className="mt-3 gap-3">
          {recommendedEvents.map((event) => (
            ...
            >
              ...
              <Text
                className={`mt-1 font-manrope ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}
              >
                {event.date}
              </Text>
              <Text className="mt-1 font-manrope-medium text-emerald-500">
                {event.price}
              </Text>
            </View>
          ))}
